...
\begin{axis}[
  ...
  symbolic x coords={Minimal,Core,Full,25-step,Scaled},xtick=data,
...
\addplot[fill=green!55!black,draw=green!35!black]
 coordinates {(Minimal,1.974) (Core,2.752) (Full,10.256)
              (25-step,3.433) (Scaled,3.476)};
\draw[densely dashed,red!75!black,thick]
  (axis cs:Minimal,3) -- (axis cs:Scaled,3)
  node[pos=.10,above,font=\tiny] {$3\times$ gate};
\end{axis}
\end{tikzpicture}
\end{minipage}
\caption{Measured Tesla T4 progress.  Left: scaled CPU/GPU trajectories remain
close initially and separate late in the nonconverged run.  Right: warm speedups
for the minimal objective, production core, full engineering objective,
25-step solve, and 100-step scaled solve.}
...
$s_I=10^5$ on the same T4.  CPU and GPU started identically and remained close
through roughly 40 iterations, then followed different floating-point-sensitive
paths.  Both reached the 100-iteration cap: CPU used 140 evaluations and ended
...
must extend convergence; backend validation should require a matching early
trajectory prefix, while solver acceptance should use stationary final physical
metrics rather than identical late iterates in this nonconvex problem.
\endgroup
