...
results_2d = []  # 2D list: rows = orders, columns = runs
feasible_results_2d = []  # 2D list: rows = orders, columns = feasible runs only

# Loop over different orders from 1 to 10
for order in range(1, 11):
    
    order_results = []  # Store results for this order
    feasible_order_results = []  # Store only feasible results for this order
    ...
    for run in range(10):
        ...
        # Check feasibility: if Jcsdist.shortest_distance() < 0.01, then it's infeasible
        final_cs_distance = Jcsdist.shortest_distance()
        is_feasible = final_cs_distance >= 0.01
        
        # Store result for this run (you can store whatever metrics you want)
        run_result = {
            'order': order,
            'run': run + 1,
            'final_flux': Jf.J(),
            ...
            'final_cs_distance': final_cs_distance,
            'is_feasible': is_feasible,
            'output_dir': OUT_DIR
        }
        order_results.append(run_result)
        
        # If feasible, add to feasible results
        if is_feasible:
            feasible_order_results.append(run_result)
            print(f"FEASIBLE: Run {run+1} for order {order}")
        else:
            print(f"INFEASIBLE: Run {run+1} for order {order}")
        
        print(f"Completed run {run+1} for order {order}")
    
    # Add this order's results to both 2D lists
    results_2d.append(order_results)
    feasible_results_2d.append(feasible_order_results)
    print(f"Completed all runs for order {order} ({len(feasible_order_results)}/{len(order_results)} feasible)")
    print("Raw data")
    print(results_2d)
    print("Pruned data")
    print(feasible_results_2d)