...
\begin{table}[htbp]
  \centering
  \caption{Confirmed progression on the minimal T4 workload.}
  \begin{tabular}{lrrr}
    \toprule
    Path & Target tile & Source tile & Median (ms) \\
    \midrule
    Original autodiff & 128  & 256  & 21.483 \\
    Tuned autodiff    & 512  & 1200 & 12.489 \\
    Custom VJP        & 1024 & 1200 &  7.979 \\
    CPU reference     & --   & --   & 15.751 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Production-scale benchmark implementation}

Revision following the minimal T4 study adds a dedicated production runner and
Colab notebook for the stress dimensions: six base coils, Fourier order 12, 180
quadrature points, and a $128\times128$ half-period surface, totaling 70,778,880
target--source interactions.  It screens 25
custom-VJP tile pairs, confirms the fastest three, uses seven synchronized
one-thread CPU samples, and exports the winning trace, memory profile, raw JSON,
and an automatically evaluated gate summary.  The $3\times$ speed criterion is
reported but does not block artifact export.  This benchmark deliberately
measures the currently supported quadratic-flux-plus-length core; distance and
curvature terms are labeled as deferred, so its result will not be presented as
complete production-objective performance.  The next decision will be based on
the returned production archive, not extrapolation from the minimal case.
...
\section{Remaining issue backlog}

The completed core-objective milestones remain in version history.  The active
sequence is now:
\begin{enumerate}
  \item Analyze the production archive and apply its profiling decision.
  \item Port curvature, arclength, coil--coil, and coil--surface terms.
  ...
  \item Profile the complete production objective and specialize bottlenecks.
...
\begin{enumerate}[label={\lbrack\arabic*\rbrack}]
  \item SIMSOPT repository, \url{https://github.com/hiddenSymmetries/simsopt}.
  \item SIMSOPT coil optimization documentation,
        \url{https://simsopt.readthedocs.io/latest/example_coils.html}.
  \item JAX, ``Default dtypes and the X64 flag,''
        \url{https://docs.jax.dev/en/latest/default_dtypes.html}.
  \item JAX, ``Benchmarking JAX code,''
