...
The next implementation will expose scaled, named constraint residuals from the
GPU objective and add a Powell--Hestenes--Rockafellar augmented-Lagrangian outer
loop while retaining SciPy for the controlled inner solve.  Multipliers and
penalty parameters must be stored per stage, and the same absolute, parity,
evaluation, field-quality, speed, and VTK contracts remain in force.  A native
L-BFGS solver remains deferred until that formulation reaches a common feasible
stationary solution.
\endgroup
