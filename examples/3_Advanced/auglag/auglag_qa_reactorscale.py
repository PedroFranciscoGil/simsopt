...
for order in orders:
    for R1_mult in R1_multipliers:
        ...
        # Initialize coils with current order and R1 multiplier
        coils = initialize_coils_simple(s, ncoils=ncoils, regularization=regularization_circ(a), 
                                      order=order, R1_multiplier=R1_mult)
        ...
        OUT_DIR = f"./output_paper/qa_{order}_{R1_mult}_ncoils{ncoils}_lengthtarget{LENGTH_TARGET}_fluxthreshold{FLUX_THRESHOLD}_ccthreshold{CC_THRESHOLD}_csthreshold{CS_THRESHOLD}_mscthreshold{MSC_THRESHOLD}_curvaturethreshold{CURVATURE_THRESHOLD}_forcethreshold{FORCE_THRESHOLD}/"
        os.makedirs(OUT_DIR, exist_ok=True)

        # Save the biot-savart field data
        bs = BiotSavart(coils)
        curves = [c.curve for c in coils]
        coils_to_vtk(coils, OUT_DIR + f"coils_init_qa_{order}_{R1_mult}_reactorscale")
        ...
        s_plot.to_vtk(OUT_DIR + f"surf_init_qa_{order}_{R1_mult}_reactorscale", extra_data=pointData)
        ...
        bs.save(OUT_DIR + f"biot_savart_optimized_auglag_qa_{order}_{R1_mult}_reactorscale.json")
        end_time = time.time()
        ...
        print("Forces:")
        print(",".join(f"{f:.2e}" for f in force))
        coils_to_vtk(coils, OUT_DIR + f"coils_optimized_auglag_qa_{order}_{R1_mult}_reactorscale")
        bs.set_points(s_plot.gamma().reshape((-1, 3)))
        calculate_modB_on_major_radius(bs, s_plot)
        bs.set_points(s_plot.gamma().reshape((-1, 3))) 
        ...
        pointData = {"B_N": np.sum(bs.B().reshape((qphi, qtheta, 3)) *
                ...
                "modB": bs.AbsB().reshape((qphi, qtheta, 1))}
        s_plot.to_vtk(OUT_DIR + f"surf_optimized_auglag_qa_{order}_{R1_mult}_reactorscale", extra_data=pointData)
