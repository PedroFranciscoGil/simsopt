...
r"""
This script performs coil optimization for the W7-X coil design using the Augmented Lagrangian Method (ALM) with a Gaussian perturbation to generate the initial coil set. It has additional check for reasonable average Bdotn/B; results won't be saved if above threshold. 
"""
...
if in_github_actions:
    ...
    nphi = 32
    ntheta = 32
    MAXITER = 4  # 1000 for high-resolution
    MAXITER_lag = 4  # 40 for high-resolution
...
# Threshold for B·n/B - if exceeded, output will not be saved
Ave_BdotN_over_B_THRESHOLD = 1e-3  
...
# Check B·n/B threshold before saving paraview output
bs_pert.set_points(s.gamma().reshape((-1, 3)))
BdotN = np.mean(np.abs(np.sum(bs_pert.B().reshape((nphi, ntheta, 3)) * s.unitnormal(), axis=2)))
avg_BdotN_over_B = BdotN / bs_pert.AbsB().mean()

print(f"Checking B·n/B threshold: {avg_BdotN_over_B:.2e} (threshold: {Ave_BdotN_over_B_THRESHOLD :.2e})")

if avg_BdotN_over_B > Ave_BdotN_over_B_THRESHOLD :
    print(f"WARNING:  B·n/B = {avg_BdotN_over_B:.2e} exceeds threshold {Ave_BdotN_over_B_THRESHOLD :.2e}")
    ...
    print("Optimization completed but output eliminated")
    print(f"Final B·n/B value: {avg_BdotN_over_B:.2e}")
  

print(f"B·n/B = {avg_BdotN_over_B:.2e} is in range. Will save optimized coils.")
