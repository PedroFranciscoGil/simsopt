...
oracle in both reverse-pass modes.  The GPU test suite contains 40 passing
...
implemented, followed by exact all-pairs distance references.  A local
full-objective smoke run reached value error $2.78\times10^{-17}$ and maximum
gradient relative error $6.38\times10^{-16}$; this validates the harness but is
not GPU performance evidence.  Rerun the updated production notebook on NVIDIA
hardware before choosing specialization: fixed-capacity neighbor lists or a
fused, tail-aware Pallas/Triton kernel remain conditional on that profile.
...
sequence is now:
\begin{enumerate}
  \item Profile the complete production objective on NVIDIA hardware.
  \item Exercise the SciPy bridge in an end-to-end trajectory comparison.
  \item Specialize distance or field bottlenecks only if the trace requires it.
