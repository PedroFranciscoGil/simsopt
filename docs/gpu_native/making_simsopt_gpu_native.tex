...
Quadratic and normalized flux and the length penalty are complete.  The first
curve-local milestone adds SIMSOPT-equivalent $L_p$ thresholded curvature,
mean-squared curvature with an outer quadratic maximum penalty, and
full-resolution arclength variation.  They reuse precomputed first- and
second-derivative Fourier bases and compose into the existing JIT objective.
Independent, deliberately active fixtures test values and combined gradients;
both reverse-pass modes also pass the integrated objective test.  Historical
flux-plus-length benchmarks retain the explicit \code{core} scope, while the
production runner now selects \code{local-engineering}.

Coil--coil and coil--surface distances remain.  Dynamic CPU candidate discovery
cannot occur inside a compiled objective; the initial GPU reference will use
all-pairs tiled penalties, followed by fixed-capacity masked neighbor lists only
if profiling requires them.
...
Retain per-workload autotuning.  The curve-local terms have subsequently been
implemented, so the production notebook now measures a
\code{local-engineering} scope and labels only the two distance penalties as
deferred.  Implement those all-pairs references next; if the complete-objective
profile remains below target, prototype a fused, tail-aware Pallas/Triton kernel
while preserving the present JAX implementation as the reference.
...
sequence is now:
\begin{enumerate}
  \item Port coil--coil and coil--surface distance terms.
