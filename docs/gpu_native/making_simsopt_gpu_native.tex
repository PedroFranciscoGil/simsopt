...
the production-sized acceptance workload.  The next experiment will sweep
larger target and source tiles, including a one-tile configuration where
memory permits, and will compare ordinary autodiff with a fused or custom VJP.
Every candidate will retain the same parity checks and will be judged on
steady-state latency and peak memory.  A synchronized CPU reference and the
larger benchmark cases are required before evaluating the $3\times$ speed
gate.
