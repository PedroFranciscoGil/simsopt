...
oracle in both reverse-pass modes.  The GPU test suite contains 46 passing
...
The integrated value-and-gradient function and generic SciPy bridge are
complete.  The coil-specific bridge now packs variables in SIMSOPT's exact
...
oracle.  Its three-iteration local smoke test matched all five evaluations,
with final objective error $2.78\times10^{-17}$ and curve-state relative error
$1.54\times10^{-16}$.

\textbf{Exit criterion:} the NVIDIA production run reaches equivalent metrics
from identical data, uses at most 10\% more physics evaluations, and preserves
the measured speed advantage.
