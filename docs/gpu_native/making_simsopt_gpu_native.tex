...
The ordinary reverse-mode implementation remains the reference.  An opt-in
\code{GpuConfig(vjp\_mode="custom")} path now supplies a custom VJP that
...
The autotuner now treats reverse-pass mode as another candidate dimension and
always confirms both the best ordinary-autodiff result and the current default.
The library default remains ordinary autodiff until NVIDIA timing and memory
evidence demonstrates an advantage.  Selection uses total value-and-gradient
time and peak memory, not forward time alone.
...
\subsection{Automated tile sweep}
...
and gradient gates.  The fastest three and the original default were then
measured for ten synchronized evaluations after three warm-up calls.

\begin{table}[htbp]
  \centering
  \caption{Confirmed tile candidates on the minimal T4 workload.}
  \begin{tabular}{rrrr}
    \toprule
    Target tile & Source tile & Tile iterations & Median (ms) \\
    \midrule
    512 & 1200 & 2  & 12.685 \\
    256 & 1200 & 4  & 12.921 \\
    128 & 1200 & 8  & 13.645 \\
...
The confirmed $512\times1200$ winner is $1.709\times$ faster than the
$128\times256$ default in the same sweep.  Its coefficient of variation is
0.484\%.  A separate trace run reproduced a $12.596\ \mathrm{ms}$ median and
the CPU-pinned, one-thread SIMSOPT reference measured $17.300\ \mathrm{ms}$
median, giving a provisional $1.364\times$ GPU speedup.  The five CPU samples
have 6.99\% variation and no affinity control, so this is not yet the tuned CPU
comparison required by the speed gate.  The one-tile $1024\times1200$
candidate took $21.236\ \mathrm{ms}$ in screening; minimizing tile count alone
therefore does not maximize throughput.

The winning trace reduces device-to-device copies from 130 to two per
evaluation and command-buffer executions from 148 to nine.  Across three
profiled calls, GPU-stream span falls from $58.71$ to $28.30\ \mathrm{ms}$ and
summed GPU activity from $53.91$ to $27.10\ \mathrm{ms}$.  Peak memory rises
from 150.27 to 256.03 MiB, but remains only 2.29\% of the JAX device limit.
Float64 parity remains at rounding scale: the objective absolute error is
$6.94\times10^{-18}$ and the curve- and current-gradient relative errors are
$5.30\times10^{-16}$ and $1.11\times10^{-16}$.

\textbf{Decision.}  Use $512\times1200$ for subsequent profiling of this
specific minimal problem on the T4.  Do not change the library-wide defaults
until the engineering and stress workloads and a stronger float64 accelerator
have been swept.  The custom Biot--Savart VJP is now implemented as an opt-in
candidate; the next NVIDIA run will jointly sweep it against ordinary autodiff.
The production-size test and tuned CPU baseline remain necessary before
evaluating the $3\times$ speed gate.
