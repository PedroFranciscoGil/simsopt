...
An initial notebook validation expression compared a Python set directly with
the final-metrics dictionary and therefore raised \code{TypeError} after the
GPU computation had completed.  The validation now compares the required set
with \code{final\_metrics.keys()}, and a notebook-source regression test fixes
this executable contract.  This correction changes neither optimizer output
nor acceptance thresholds; rerunning only the validation and download cells is
sufficient when the result file remains in the active Colab runtime.

\subsection{Measured A100 replay results}

The replay was executed on an NVIDIA A100-SXM4-40GB with JAX 0.11.1 from
revision \code{cb4b3964}.  The returned archive has SHA-256
\code{e5d0b77b2ac744ef671ab3b9d739c9e15d3088d99113024711791225c5fd3d24}.
All eight configurations pass the scientific rule: quadratic flux is below
$1.1\times10^{-5}$ and every coil quantity is within the allowed 10\% target
envelope.  Thus the sweep is scientifically validated even though its separate
CPU-reference scalar-objective diagnostic does not pass.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/device_lbfgs_replay_grid.png}
  \caption{Device-resident L-BFGS replay grid on the A100.  Patience changes
  runtime and evaluation count but not the selected state for a fixed history
  size.  The first $m=10$ compilation includes one-time runtime initialization
  and is therefore not used to infer history-size compilation scaling.}
  \label{fig:device-replay-grid}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/device_lbfgs_replay_trajectories.png}
  \caption{Quadratic-flux trajectories and selected feasible checkpoints.
  Every trajectory crosses the permitted boundary, and continued iterations
  reduce flux by more than another order of magnitude before target-aware
  stopping.}
  \label{fig:device-replay-trajectories}
\end{figure}

\begin{table}[htbp]
\centering
\small
\caption{A100 device-LBFGS replay measurements.  Time excludes compilation.}
\label{tab:device-replay-sweep}
\begin{tabular}{rrrrrr}
\toprule
$m$ & Patience & Iterations & Evaluations & Time (s) & Quadratic flux \\
\midrule
10  & 15 & 93  & 98  & 0.754 & $7.6781\times10^{-7}$ \\
10  & 25 & 103 & 110 & 0.866 & $7.6781\times10^{-7}$ \\
20  & 15 & 79  & 81  & 0.675 & $7.5684\times10^{-7}$ \\
20  & 25 & 89  & 92  & 0.768 & $7.5684\times10^{-7}$ \\
50  & 15 & 59  & 60  & 0.559 & $8.0059\times10^{-7}$ \\
50  & 25 & 69  & 70  & 0.665 & $8.0059\times10^{-7}$ \\
100 & 15 & 59  & 60  & 0.662 & $8.0059\times10^{-7}$ \\
100 & 25 & 69  & 70  & 0.752 & $8.0059\times10^{-7}$ \\
\bottomrule
\end{tabular}
\end{table}

History 20, patience 15 is the archive winner because it gives the lowest
qualified quadratic flux.  Relative to patience 25 it selects the identical
checkpoint while using 11 fewer evaluations and 12.1\% less warm solve time.
Its 0.675~s replay is 2.8\% faster than the 0.694~s host-driven GPU SciPy solve
measured in the preceding qualification.  History 50, patience 15 is a useful
speed-first alternative: 0.559~s and 60 evaluations, 17.2\% faster than the
chosen $m=20$ replay, at the cost of 5.8\% higher quadratic flux.  Because both
are well inside all physical targets, this is an explicit quality--latency
choice rather than a validity distinction.  The production default remains
$m=20$ and adopts patience 15; $m=50$ should be exposed as an opt-in throughput
setting until it is reproduced on additional starting configurations.

\begin{table}[htbp]
\centering
\small
\caption{Final CPU-reference and device-winner quantities.  Relative
differences diagnose trajectory agreement and are not target errors.}
\label{tab:device-replay-qoi}
\begin{tabular}{lrrr}
\toprule
Quantity & CPU SciPy & Device $m=20,p=15$ & Relative difference \\
\midrule
Penalty objective & $1.0767\times10^{-4}$ & $2.6945\times10^{-4}$ & 150.25\% \\
Quadratic flux & $8.6156\times10^{-7}$ & $7.5684\times10^{-7}$ & 12.15\% \\
Mean $|B\cdot n|/|B|$ & $1.3566\times10^{-3}$ & $1.2896\times10^{-3}$ & 4.94\% \\
RMS $|B\cdot n|/|B|$ & $1.7413\times10^{-3}$ & $1.6352\times10^{-3}$ & 6.09\% \\
Maximum $|B\cdot n|/|B|$ & $6.1185\times10^{-3}$ & $5.7371\times10^{-3}$ & 6.23\% \\
Total base-coil length (m) & 17.8202 & 17.6366 & 1.03\% \\
Minimum coil--coil distance (m) & 0.13963 & 0.14484 & 3.74\% \\
Minimum coil--surface distance (m) & 0.28019 & 0.27112 & 3.24\% \\
Maximum curvature ($\mathrm{m}^{-1}$) & 5.0389 & 4.8198 & 4.35\% \\
Maximum mean-squared curvature ($\mathrm{m}^{-2}$) & 5.4792 & 5.1541 & 5.93\% \\
\bottomrule
\end{tabular}
\end{table}

The 150\% penalty-objective difference explains why the archive-level
\code{all\_gates\_passed} field is false, but it does not contradict the user's
validation rule.  That scalar combines soft penalties whose trade-offs differ
between valid trajectories; all explicit physical quantities meet their target
envelopes.  Future result schemas should label this condition as a technical
trajectory diagnostic rather than an overall scientific failure.  The archive
also contains the selected surface VTS with $B\cdot n/|B|$ and the corresponding
coil VTU for visual inspection.

\subsection{Implemented end-to-end device-resident augmented Lagrangian}

The next implementation removes SciPy and the Python outer loop from the GPU
path.  A new \code{DeviceAugmentedLagrangian} lowers the complete nested solve
to one JAX executable.  The compiled computation includes the AL outer loop,
dynamic multiplier and penalty vectors, residual mapping and scale
continuation, L-BFGS inner iterations, the two-loop recursion, Armijo
backtracking, and the absolute-or-relative stationarity safeguard.  Only the
final state and fixed-size outer history cross back to the host; the measured
host-callback count is therefore zero by construction.
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, minimum height=8mm,
              text width=31mm, fill=blue!6},
  gpu/.style={box, fill=green!10},
  arrow/.style={-{Latex[length=2mm]}, thick}
]
  \node[box] (cpuouter) {Python AL\\outer loop};
  \node[box, below=of cpuouter] (scipy) {SciPy L-BFGS-B\\inner solve};
  \node[box, below=of scipy] (cpuphysics) {CPU-pinned JAX\\physics/gradient};
  \draw[arrow] (cpuouter) -- (scipy);
  \draw[arrow] (scipy) -- node[right, font=\scriptsize]{host calls} (cpuphysics);

  \node[gpu, right=20mm of scipy] (compiled) {single compiled\\GPU executable};
  \node[gpu, below=of compiled] (outer) {device AL updates\\and safeguards};
  \node[gpu, below=of outer] (inner) {device L-BFGS and\\Armijo search};
  \node[gpu, below=of inner] (physics) {GPU coil physics\\and reverse mode};
  \draw[arrow] (compiled) -- (outer);
  \draw[arrow] (outer) -- (inner);
  \draw[arrow] (inner) -- (physics);
  \draw[arrow, bend left=38] (physics.east) to (outer.east);
\end{tikzpicture}
\caption{Execution boundary for the no-refinement end-to-end comparison.  The
CPU reference retains the established host-controlled algorithm; every GPU
optimization control flow edge remains within the compiled executable.}
\label{fig:end-to-end-device-al-architecture}
\end{figure}

The matched production benchmark uses the engineering problem from the same
circular order-eight, 207-variable initial state.  Both paths use at most eight
outer iterations and 300 inner iterations per stage; $\mu_0=10$, $\tau=2$,
$\mu_{\max}=10^{12}$; smooth-absolute local residuals with $\epsilon=0.1$;
family-$\ell_2$ initial scales, square-root-count scale floors and scale factor
0.5; global penalty updates; relative stationarity threshold 0.01; and float64
physics.  The established CPU reference retains SciPy's 100-pair history, while
the device solver uses the A100-qualified 20-pair history.  This intentionally
compares the production configuration of each optimizer; iteration and
physics-evaluation counts accompany time so algorithmic work remains visible.

No direct-flux refinement is permitted in this workflow.  Schemas 1--2 compute
final metrics immediately at the two terminal AL states with the SIMSOPT CPU
oracle; schema~3 applies the documented target-aware selection only among the
initial state and completed AL outer states.  The
result stores the penalty objective, quadratic flux, mean/RMS/maximum
$|B\cdot n|/|B|$, currents and all coil-constraint measurements for both paths.
Each endpoint is independently tested against the 10\% physical target policy;
CPU/device trajectory agreement is a separate diagnostic.  Both final surfaces
are exported as VTS with signed and absolute $B\cdot n/|B|$, and both coil sets
as VTU.

Timing has two explicit scopes.  CPU cold end-to-end time is CPU objective
compilation plus the original AL solve.  GPU cold end-to-end time is compilation
of the entire nested executable plus its first synchronized execution.  Warm
GPU time is the median of three synchronized executions from the identical
initial state.  The archive also retains compilation, every execution sample,
total inner iterations and objective/gradient evaluations.  This prevents
kernel-only timing from being presented as end-to-end performance.

The new Colab workflow produces
\code{simsopt-end-to-end-device-al.zip}.  Its analyzer will generate cold-time
and work-count bars, outer convergence curves, and final magnetic-metric bars.

\subsection{A100 end-to-end result: fast, but scientifically rejected}

The first no-refinement archive was returned on 14 September 2026 from
revision \code{c801a6e0}.  Its SHA-256 digest is
\code{25ac2a5cf335a01633483fae1e69385112f12c42e67b531a17376968821d9e78}.
The runtime was an NVIDIA A100-SXM4-40GB with JAX 0.11.1, SciPy 1.16.3,
Python 3.13.15, float64 enabled and one OpenMP thread.  Both paths began from
the identical canonical circular order-eight configuration.  The archive
contains nonempty VTS and VTU files for both endpoints; each VTS exposes
\code{B\_dot\_n\_over\_abs\_B}, its absolute value, \code{B\_dot\_n}, and
\code{abs\_B} as surface arrays.

The CPU path compiled in 1.946~s and ran for 800.270~s, for 802.216~s cold
end-to-end.  It executed 421 inner iterations and 1,114 value/gradient
evaluations over two AL stages, then correctly stopped when the second inner
solve reached its 300-iteration limit without satisfying the stationarity
safeguard.  The complete GPU executable compiled in 5.063~s.  Its three
execution samples were 1.743, 1.702, and 1.687~s (1.702~s median), with 38
inner iterations and 318 evaluations over two AL stages.  This gives nominal
cold and warm ratios of $117.9\times$ and $470.2\times$.  The GPU advantage
contains both faster evaluations and $3.50\times$ less algorithmic work, so it
must not be interpreted as kernel speed alone.

\begin{table}[htbp]
\centering
\small
\caption{No-refinement A100 endpoints.  The target flux is
$10^{-5}$, with an allowed boundary of $1.1\times10^{-5}$.}
\label{tab:end-to-end-al-unbounded-qoi}
\begin{tabular}{lrr}
\toprule
Quantity & CPU SciPy AL & GPU-native AL \\
\midrule
Scalar objective & $7.28381\times10^{-3}$ & $6.87271\times10^{-3}$ \\
Quadratic flux & $7.26889\times10^{-3}$ & $1.37337\times10^{-3}$ \\
Mean $|B\cdot n|/|B|$ & 0.112602 & 0.531810 \\
RMS $|B\cdot n|/|B|$ & 0.135023 & 0.623111 \\
Maximum $|B\cdot n|/|B|$ & 0.264221 & 0.999930 \\
Total base-coil length (m) & 14.9170 & 5499.3317 \\
Minimum coil--coil distance (m) & 0.144847 & 0.790087 \\
Minimum coil--surface distance (m) & 0.302014 & 5.871547 \\
Maximum curvature ($\mathrm{m}^{-1}$) & 5.00099 & 0.959073 \\
Maximum mean-squared curvature ($\mathrm{m}^{-2}$) & 5.00102 & 0.0113584 \\
\bottomrule
\end{tabular}
\end{table}

The CPU endpoint satisfies every engineering target within the requested 10\%
allowance, but misses the quadratic-flux boundary by a factor of 661.  The GPU
endpoint also reports zero represented engineering violations and misses the
flux boundary by a factor of 125; nevertheless it is physically degenerate.
Its four base coils have lengths 1251, 1782, 959, and 1506~m, compared with
3.58--3.88~m on the CPU.  Enlarging the coils makes the distance and curvature
hinges vanish, while the $10^{-6}$ linear length term contributes only about
$5.5\times10^{-3}$ to the objective.  The device solver can therefore accept
a 56.1-scaled-coordinate step that genuinely lowers the modeled objective.
This is not an objective-evaluation bug; it exposes a noncoercive feasible
model and an unmatched practical distinction between SciPy's local trajectory
and the device Armijo trajectory.

\textbf{Decision.}  Reject this run as the scientific end-to-end
qualification and retain its timing only as diagnostic evidence.  The
subsequent implementation now adds identical physical design-envelope bounds
to the CPU and device inner solvers: free currents are restricted to
$0.5$--$1.5$ times their starting values and every Fourier coefficient to
$\pm0.25$~m about its circular-start value.  Projected-gradient convergence,
bound-filtered directions and clipped Armijo trials execute fully on device.
The next archive will rerun from the same circles with no refinement.
Qualification still requires each endpoint to meet the
$1.1\times10^{-5}$ flux boundary and every engineering target; speedup will be
reported only after those gates pass.  A per-coil maximum-length residual is a
possible later physical constraint, but it will not be silently inserted into
the stated engineering problem merely to improve this benchmark.

\subsection{Bounded A100 rerun: geometry fixed, stationarity blocks progress}

The matched-bound archive from revision \code{ca748727} has the SHA-256 digest
\begin{center}
\small\nolinkurl{96021611e05c1945d184184169be4edb1fbee4ea4775c834731f9bc4bded422a}
\end{center}
It uses the same A100, software stack, initial circles, problem resolution and
no-refinement rule as the first run.  All 207 optimizer variables are bounded
identically on CPU and GPU.  Neither selected endpoint is active at a bound,
but the bounded device line search prevents the previous escape to kilometre-
scale coils.  The GPU endpoint now has 14.832~m total base-coil length.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_2_performance.png}
\caption{Bounded A100 time and work.  CPU cold time is 465.060~s; GPU cold
time is 11.663~s and its warm median is 6.658~s.  The nominal $39.88\times$
cold and $69.57\times$ warm ratios remain diagnostic because neither endpoint
passes the squared-flux gate.}
\label{fig:end-to-end-al-performance}
\end{figure}

The CPU reference exhausts all 300 inner iterations in its first AL stage,
using 635 evaluations, and the hard stationarity safeguard rejects the stage.
The GPU completes a stationary three-iteration first stage, then exhausts the
second stage's 300-iteration budget.  It uses 1,217 evaluations in total and
the same safeguard stops further outer updates.  Thus the GPU performs
$1.92\times$ more value/gradient evaluations yet remains $69.57\times$ faster
in the warm optimization measurement.  Compilation is 1.877~s on CPU and
4.994~s for the complete GPU program.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_2_convergence.png}
\caption{Bounded outer histories.  The GPU reduces the base objective from
$1.762\times10^{-2}$ to $3.683\times10^{-3}$ in its two stages.  Its strict
local-residual norm grows slightly in the second stage, although all final
engineering quantities remain within the user's 10\% target envelope.}
\label{fig:end-to-end-al-convergence}
\end{figure}

\begin{table}[htbp]
\centering
\small
\caption{Matched-bound, no-refinement endpoints.  Target validation is based
on the explicit quantities, not gradient size.}
\label{tab:end-to-end-al-bounded-qoi}
\begin{tabular}{lrr}
\toprule
Quantity & CPU SciPy AL & GPU-native AL \\
\midrule
Scalar objective & $5.14474\times10^{-2}$ & $3.68317\times10^{-3}$ \\
Quadratic flux & $2.30349\times10^{-2}$ & $3.66831\times10^{-3}$ \\
Mean $|B\cdot n|/|B|$ & 0.192088 & 0.0806408 \\
RMS $|B\cdot n|/|B|$ & 0.222743 & 0.101082 \\
Maximum $|B\cdot n|/|B|$ & 0.448399 & 0.230422 \\
Total base-coil length (m) & 13.1413 & 14.8316 \\
Minimum coil--coil distance (m) & 0.142470 & 0.169673 \\
Minimum coil--surface distance (m) & 0.182914 & 0.296002 \\
Maximum curvature ($\mathrm{m}^{-1}$) & 4.77503 & 4.57415 \\
Maximum mean-squared curvature ($\mathrm{m}^{-2}$) & 5.00113 & 5.00102 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/end_to_end_al_schema_2_final_metrics.png}
\caption{Bounded direct-AL magnetic quantities.  The GPU is consistently
better than CPU in this stopped run, but its quadratic flux is still about
333 times the allowed $1.1\times10^{-5}$ boundary.}
\label{fig:end-to-end-al-final-metrics}
\end{figure}

The GPU endpoint passes all four engineering target envelopes: its minimum
coil--surface distance is 0.2960~m, safely above the allowed 0.27~m boundary,
and its curvature quantities are also accepted.  Its quadratic flux fails by
a factor of 333.5.  The CPU endpoint fails both flux (a factor of 2094 above
the boundary) and coil--surface distance (0.1829~m).  The stored raw gradients
are large, but that alone is not a scientific failure under the agreed policy.
Both nonempty VTS files retain signed and absolute $B\cdot n/|B|$ arrays, and
both coil sets are present as VTU.

\textbf{Decision.}  Keep the matched design envelope; it solved the runaway
geometry defect.  Stop using inner stationarity as a hard veto when a finite,
budget-limited coil step has useful physical progress.  Schema~3 therefore
makes stationarity diagnostic, permits the AL outer loop to continue through
its fixed budget, and retains every outer design on device.  After the solve,
both backends apply the same target-aware checkpoint rule: prefer complete
scientific validity, then engineering-valid states, then minimum target
violation, and minimize quadratic flux within the chosen tier.  This is
checkpoint selection inside one AL workflow, not a refinement solve.  Its
CPU-oracle selection cost is stored separately; solver cold end-to-end remains
compilation plus optimization, avoiding an order-dependent charge for the
oracle's first compilation.  The next A100 archive must still pass the
explicit flux and engineering gates before any speedup is accepted.

\subsection{Budgeted A100 result: strong timing, premature success}

The schema~3 archive from revision \code{fcd53d61} has the SHA-256 digest
\begin{center}
\small\nolinkurl{d25186f5620581ba98fef0ad5b636c46bdb41cf86ec9848738ef8818714f1fdd}
\end{center}
It reproduces the same A100 float64 environment and matched 207-variable
design envelope.  Stationarity is diagnostic for accepting budget-limited
inner stages, and each solver retains every completed outer design for the
same target-aware CPU-oracle selection.  The archive and selection contracts
reproduce, and all VTS/VTU payloads are nonempty with the required surface
normal-field arrays.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_3_performance.png}
\caption{Schema~3 solver time and work.  CPU cold end-to-end is 3191.359~s;
GPU cold is 24.151~s and the synchronized warm median is 19.137~s.  The GPU
uses 3,536 evaluations versus 4,422 on CPU.}
\label{fig:end-to-end-al-schema3-performance}
\end{figure}

The resulting nominal acceleration is $132.14\times$ cold and $166.67\times$
warm.  CPU compilation takes 1.839~s and optimization 3189.519~s; GPU
compilation takes 5.014~s, followed by execution samples of 19.137, 19.249 and
19.079~s.  CPU performs 1,223 inner iterations across five outer stages; GPU
performs 903 across four.  Target-aware validation/selection costs are stored
separately (5.805~s CPU and 0.956~s GPU), so first-use CPU-oracle compilation
does not contaminate solver timing.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_3_convergence.png}
\caption{Budgeted outer trajectories.  Allowing finite nonstationary stages
produces continued engineering-feasibility progress, but both solvers stop
before the configured eight-stage maximum once strict residuals vanish and
the relative stationarity test passes.}
\label{fig:end-to-end-al-schema3-convergence}
\end{figure}

The selector chooses CPU outer stage~5 and GPU stage~4.  In both cases this is
the terminal state and the first-class checkpoint machinery does not disguise
a degraded endpoint.  Both selected designs pass every engineering target
under the 10\% rule, and their physical constraints agree well: coil--coil
distance differs by 2.08\%, coil--surface distance by 0.064\%, maximum
curvature by 2.16\%, maximum mean-squared curvature by $2.24\times10^{-6}$\%,
and total base-coil length by 6.24\%.  No variable is active at its design-
envelope bound.

\begin{table}[htbp]
\centering
\small
\caption{Schema~3 target-aware direct-AL designs.}
\label{tab:end-to-end-al-schema3-qoi}
\begin{tabular}{lrr}
\toprule
Quantity & CPU outer 5 & GPU outer 4 \\
\midrule
Scalar objective & $1.50673\times10^{-2}$ & $1.77181\times10^{-3}$ \\
Quadratic flux & $1.50531\times10^{-2}$ & $1.75677\times10^{-3}$ \\
Mean $|B\cdot n|/|B|$ & 0.158445 & 0.0578919 \\
RMS $|B\cdot n|/|B|$ & 0.184964 & 0.0732176 \\
Maximum $|B\cdot n|/|B|$ & 0.375966 & 0.203306 \\
Total base-coil length (m) & 14.1582 & 15.0416 \\
Minimum coil--coil distance (m) & 0.159805 & 0.163126 \\
Minimum coil--surface distance (m) & 0.300166 & 0.299973 \\
Maximum curvature ($\mathrm{m}^{-1}$) & 4.81122 & 4.70748 \\
Maximum mean-squared curvature ($\mathrm{m}^{-2}$) & 5.0009934 & 5.0009935 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/end_to_end_al_schema_3_final_metrics.png}
\caption{Schema~3 magnetic metrics.  The GPU endpoint is substantially better
than CPU, but its quadratic flux remains 159.7 times the allowed
$1.1\times10^{-5}$ boundary; CPU is 1368.5 times above it.}
\label{fig:end-to-end-al-schema3-final-metrics}
\end{figure}

\textbf{Decision.}  The budgeted acceptance and checkpoint mechanisms are
retained: they improve progress without weakening the user's physical
validation rule.  The remaining premature termination is caused by applying
the relative-stationarity convergence rule after the strict engineering
residual becomes zero.  Schema~4 adds a minimum continuation depth of eight
outer stages to both host and device AL solvers.  Before stage~8, a stationary,
strictly feasible inner result is allowed to update continuation state but not
terminate the solve.  The best of all outer states is still selected, so later
stages cannot erase an earlier target-valid design.  This directly tests
whether continued AL minimization can close the remaining flux gap without a
separate refinement phase.

\subsection{Eight-stage A100 result: improvement without qualification}

The schema~4 archive from revision \code{c4bdc0c5} has the SHA-256 digest
\begin{center}
\small\nolinkurl{379e1a1182b28e7c971d9881a5d4b735b14513dd71100399368a74a2fc7f65e0}
\end{center}
It contains the complete no-refinement comparison, including nine candidate
states per backend (the circular start and eight outer endpoints), nonempty
coil VTU files, and surface VTS files with signed and absolute
$B\cdot n/|B|$, $B\cdot n$, and $|B|$ arrays.  The run again used float64 on
an NVIDIA A100-SXM4-40GB, four order-8 base coils, 16 physical coils, 207
optimizer variables, 120 coil quadrature points, and a $64\times64$ surface.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_4_performance.png}
\caption{Complete eight-stage AL cost.  CPU cold solver time is 4645.006~s;
GPU cold time is 41.605~s and its synchronized warm median is 36.162~s.  The
GPU performs 6,702 objective/gradient evaluations versus 6,235 on CPU.}
\label{fig:end-to-end-al-schema4-performance}
\end{figure}

The nominal ratios are $111.65\times$ cold and $128.40\times$ warm.  CPU
compilation takes 1.897~s and optimization 4643.109~s; GPU compilation takes
5.443~s, followed by 36.162, 36.390 and 36.116~s executions.  CPU accumulates
1,826 inner iterations and GPU 1,804.  These are genuine end-to-end solver
measurements with no refinement phase, but they remain diagnostic rather than
a qualified speedup because the physical flux gate fails.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.98\textwidth]{figures/end_to_end_al_schema_4_convergence.png}
\caption{All eight outer stages now execute.  Relative to schema~3, the GPU
quadratic flux improves by $4.22\times$ and the CPU flux by $1.38\times$.
Both target-aware selectors choose the eighth endpoint.}
\label{fig:end-to-end-al-schema4-convergence}
\end{figure}

\begin{table}[htbp]
\centering
\small
\caption{Independent CPU-oracle metrics for the selected schema~4 endpoints.}
\label{tab:end-to-end-al-schema4-qoi}
\begin{tabular}{lrr}
\toprule
Quantity & CPU outer 8 & GPU outer 8 \\
\midrule
Scalar objective & $1.09308\times10^{-2}$ & $4.32079\times10^{-4}$ \\
Quadratic flux & $1.09165\times10^{-2}$ & $4.16518\times10^{-4}$ \\
Mean $|B\cdot n|/|B|$ & 0.137409 & 0.0300371 \\
RMS $|B\cdot n|/|B|$ & 0.161502 & 0.0362835 \\
Maximum $|B\cdot n|/|B|$ & 0.326054 & 0.123136 \\
Total base-coil length (m) & 14.2767 & 15.5614 \\
Minimum coil--coil distance (m) & 0.160666 & 0.149579 \\
Minimum coil--surface distance (m) & 0.300240 & 0.307747 \\
Maximum curvature ($\mathrm{m}^{-1}$) & 4.66836 & 4.75271 \\
Maximum mean-squared curvature ($\mathrm{m}^{-2}$) & 5.0010000 & 5.0010006 \\
\bottomrule
\end{tabular}
\end{table}

Both designs pass all four engineering target envelopes.  Their reported coil
quantities agree within 9.00\%, while their magnetic metrics do not agree: the
different L-BFGS implementations follow materially different nonconvex
trajectories.  This trajectory difference is retained as a technical
diagnostic and does not replace endpoint validation.  GPU quadratic flux is
still 37.87 times the $1.1\times10^{-5}$ allowed boundary; CPU is 992.41 times
above it.  The GPU finishes with a strict local-residual infinity norm of
$6.43\times10^{-4}$ and therefore does not report mathematical convergence,
although its actual engineering quantities pass the agreed 10\% rule.  Large
gradient values remain diagnostic, consistent with the validation policy.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.88\textwidth]{figures/end_to_end_al_schema_4_final_metrics.png}
\caption{Schema~4 final magnetic quantities with the allowed quadratic-flux
boundary shown explicitly.  Full continuation narrows but does not close the
scientific gap.}
\label{fig:end-to-end-al-schema4-final-metrics}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\textwidth]{figures/end_to_end_al_schema_4_surface_field.png}
\caption{Signed $(B\cdot n)/|B|$ decoded from the exported final surface VTS
files on a common color scale.  The GPU design has a visibly weaker residual
field, consistent with its lower quadratic flux and normalized-field metrics.}
\label{fig:end-to-end-al-schema4-surface}
\end{figure}

\textbf{Decision.}  Keep the complete eight-stage comparison and all existing
scientific gates, but remove a mismatch between optimization and validation.
Schema~4 drives the engineering residuals nearly to the nominal boundaries
0.1~m, 0.3~m, $5~\mathrm{m}^{-1}$, and $5~\mathrm{m}^{-2}$ even though the
agreed 10\% validation envelope accepts 0.09~m, 0.27~m,
$5.5~\mathrm{m}^{-1}$, and $5.5~\mathrm{m}^{-2}$.  Schema~5 makes those four
accepted boundaries the exact zero set of both CPU and GPU AL residuals while
preserving their smoothing widths, the original quadratic-flux plus length
objective, design bounds, iteration budgets, and no-refinement rule.  The
archive records and the analyzer reproduces all four optimizer boundaries.
This is not a relaxation of validation; it makes the mathematical problem
match the previously approved scientific policy and releases unnecessary
constraint pressure for magnetic optimization.
