...
No direct-flux refinement is permitted in this workflow.  Final metrics are
computed immediately at the two AL endpoints with the SIMSOPT CPU oracle.  The
...
\caption{Measured A100 end-to-end time and work.  These are implementation
measurements, not a validated scientific speedup: the endpoints fail the flux
target and the GPU endpoint leaves the intended coil-design envelope.}
...
\caption{Two-stage outer histories.  The GPU's zero engineering-residual norm
is real for the residuals currently represented, but misleading as a design
qualification because those residuals contain lower-distance and
upper-curvature conditions but no upper coil-length envelope.}
...
\caption{Final magnetic quantities before any refinement.  Neither backend
meets the requested quadratic-flux target, and the normalized normal field
shows that the lower GPU quadratic flux does not describe an acceptable
surface-field solution.}
