...
"""
qss_script_jax_qa.py — QSS Optimisation for Quasi-Axisymmetric (QA) configs (JAX variant)
=========================================================================================

Same MODERN QSS machinery as qss_script_jax_qi.py (AL1 + AL2 augmented-
Lagrangian with the corrected inner-AL2 Eq.(18) adjoint, B1 truncated-SVD
regularisation, the arclength gauge fix, central finite differences,
MPI-safe dual updates, dropped-active multiplier diagnostics, per-iteration
checkpointing, and the FD-Hessian default) but targeting the SAME QA
equilibrium and the SAME coil/equilibrium targets as qss_script_jax.py.

Equilibrium (from qss_script_jax.py):
- QuasisymmetryRatioResidual (helicity m=1, n=0) — main QA objective
- aspect ratio -> ASPECT_TARGET, mean iota -> IOTA_TARGET,
  vacuum well -> VACUUM_WELL_TARGET (target-style equality constraints)
...
# Elongation target (same source as qss_script_jax.py).
...
_qss_argparser = argparse.ArgumentParser(
    description="QSS Optimisation for QA configurations with JAX pipeline.",
)
...
# Coil geometry constants (QA, same as qss_script_jax.py)
...
def main():
    proc0_print("")
    proc0_print(SEP)
    proc0_print("  QSS (Quasi-Single-Stage) QA Optimization -- AL1 [JAX pipeline]")
    proc0_print(SEP)
    proc0_print("")
    ...
    # QA omnigenity/quasisymmetry residual (main objective, returns a vector).
    qs = QuasisymmetryRatioResidual(vmec,
                                    np.arange(0, 1.01, 0.1),
                                    helicity_m=1, helicity_n=0)

    # ---------------------------------------------------------------------------
    # QA equilibrium targets (same as qss_script_jax.py)
    # ---------------------------------------------------------------------------
    VACUUM_WELL_TARGET   = 0.015   # 0.5%
    ASPECT_TARGET        = 6.0
    IOTA_TARGET          = 0.47
    ELONGATION_THRESHOLD = 6
    ...
    # ---------------------------------------------------------------------------
    # Coil optimisation parameters (AL2 subproblem) — same as qss_script_jax.py
    # ---------------------------------------------------------------------------
    LENGTH_TARGET        = 3.5
    ...
    # No coil-force constraint (matches qss_script_jax.py coil targets).
    al2_c_list = _coil_objs["c_list"]
    AL2_CONSTRAINT_NAMES = ["Jf", "Jccdist", "Jcsdist", "Jmscs", "Jls", "J_kappa", "Jlink"]
    ...
    EQ_CONSTRAINTS = [
        Constraint(fn=vmec.aspect,      target=ASPECT_TARGET,      label="aspect"),
        Constraint(fn=vmec.mean_iota,   target=IOTA_TARGET,        label="iota"),
        Constraint(fn=vmec.vacuum_well, target=VACUUM_WELL_TARGET, label="vacuum_well"),
        Constraint(fn=elong_opt.J,      target=0.0,                label="elongation"),
    ]
    ...
    proc0_print(f"  Targets: aspect={ASPECT_TARGET}, iota={IOTA_TARGET}, "
                f"vacuum_well={VACUUM_WELL_TARGET}, elongation<={ELONGATION_THRESHOLD}")
    proc0_print("")
    ...
    proc0_print(f"  {'Quasisymmetry':<20s}{qs_total_init:>20.6e}")
    ...
    # Compute final QA metrics (triggers vmec.run() via cached state)
    qs_total_final = float(qs.total())
    ...
    proc0_print(f"  Quasisymmetry:   {qs_total_final:.6e}")
    ...
    if mpi.proc0_world:
        final_metrics = {
            ...
            "optimization": {
                ...
                "eq18_arclength_gauge": EQ18_ARCLENGTH_GAUGE,
                "eq18_implicit_rcond": EQ18_IMPLICIT_RCOND,
            },
        }
