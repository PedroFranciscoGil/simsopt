...
"""
qss_script_jax_qa_phr.py — QSS QA optimisation with PHR inequality ALM
=======================================================================

A/B variant of qss_script_jax_qa.py. Same architecture (continuation +
...
SIGNED metrics instead of an equality/target-style AL:

  Equality/target (qss_script_jax_qa.py)       PHR (this script)
  -------------------------------------------  --------------------------------
  c = m - t   treated as c = 0                 g = m - t  treated as g <= 0
  term: (rho/2) (c + lam/rho)^2                term: (rho/2) max(0, g + lam/rho)^2
  lam <- lam + rho*c                           lam <- clip(lam + rho*g, 0, lam_max)
...
The QA equilibrium targets of qss_script_jax_qa.py (aspect -> 6.0,
mean_iota -> 0.47, vacuum_well -> 0.015, elongation <= 6) are recast as
ONE-SIDED inequalities g <= 0 for the inequality-only PHR framework:

  g_aspect = aspect - 6.0    <= 0   (aspect <= 6)
  g_iota   = 0.47  - iota    <= 0   (iota  >= 0.47)
  g_well   = 0.015 - W       <= 0   (W     >= 0.015)
  g_elong  = elong - 6.0     <= 0   (elong <= 6)
...
Key practical differences:
  ...
  - The AL1-promoted coil metrics (Jf, Jls, J_kappa) are penalty-valued
    ...
    multipliers cannot decay below zero violation.

Run `--self-test` for a fast analytic KKT check of the PHR helpers
(no VMEC, no MPI; seconds).

Initial condition: input.nfp2_QA (NFP=2, stellsym, vacuum).
Coil/equilibrium targets match qss_script_jax_qa.py (and qss_script_jax.py).
"""
...
# Elongation target (same source as qss_script_jax_qa.py / qss_script_jax.py).
# MaxElongationPen is a linear hinge max(0, e_max - t); called with t=0 it
# returns the raw elongation, which is what the PHR formulation needs.
_targets_dir = os.path.join(
    _script_dir, '..', 'auglag_for_surface', 'optimization_files')
if _targets_dir not in sys.path:
    sys.path.insert(0, _targets_dir)
from Targets import MaxElongationPen

# ---------------------------------------------------------------------------
# CLI arguments
# ---------------------------------------------------------------------------
_qss_argparser = argparse.ArgumentParser(
    description="QSS QA optimisation with PHR inequality ALM "
...
_qss_argparser.add_argument(
    "--n-continuation-steps", type=int, default=5,
    help="Number of continuation steps (default: 5). Max modes go 1..N.",
)

_qss_argparser.add_argument(
    "--vmec-input", type=str, default=None,
    help="Path to VMEC input file. Default: "
         "<script_dir>/../../2_Intermediate/inputs/input.nfp2_QA",
)
...
_qss_argparser.add_argument(
    "--finite-beta", action="store_true", default=False,
    help="Finite-beta equilibrium optimisation: impose a fixed pressure "
         "profile p(s) = pres_scale*(1 - s) (the default input is vacuum) and "
         "add the Mercier interchange stability criterion (min interior "
         "DMerc >= mercier_margin) as an extra PHR inequality constraint. "
         "Requires --pres-scale > 0.",
)

_qss_argparser.add_argument(
    "--pres-scale", type=float, default=0.0,
    help="VMEC pres_scale [Pa] for the imposed p(s) = pres_scale*(1 - s) "
         "profile when --finite-beta is set (default: 0.0). Must be > 0 for a "
         "finite-beta run; tune it for the target beta (achieved betatotal is "
         "printed). Held fixed during the optimisation.",
)

_qss_argparser.add_argument(
    "--mercier-margin", type=float, default=0.0,
    help="Mercier stability margin: require min interior DMerc >= this value "
         "(default: 0.0, i.e. DMerc >= 0, marginal interchange stability). "
         "Only used when --finite-beta is set.",
)
...
FINITE_BETA = bool(_qss_args.finite_beta)
PRES_SCALE = float(_qss_args.pres_scale)
MERCIER_MARGIN = float(_qss_args.mercier_margin)

# Finite-beta fixed pressure profile p(s) = pres_scale * sum(PRESSURE_AM_SHAPE[i]*s^i).
# Default shape is a simple peaked profile p(s) proportional to (1 - s).
PRESSURE_AM_SHAPE = [1.0, -1.0]
...
# Coil geometry constants (QA, same as qss_script_jax_qa.py / qss_script_jax.py)
...
def main():
    proc0_print("")
    proc0_print(SEP)
    proc0_print("  QSS QA Optimization -- PHR inequality AL1 "
                "[JAX pipeline]")
    proc0_print(SEP)
    proc0_print("")
    ...
    _default_vmec_input = os.path.join(
        _script_dir, '..', '..', '2_Intermediate', 'inputs', 'input.nfp2_QA')
    ...
    # ---------------------------------------------------------------------------
    # Finite-beta equilibrium: impose a fixed pressure profile p(s) =
    # pres_scale * (1 - s). The default input is a vacuum equilibrium (AM=0),
    # so the Mercier criterion added below is only meaningful once p > 0. The
    # profile is held FIXED during the optimisation (pres_scale is a VMEC
    # scalar DOF that stays fixed by default — only the surface is varied).
    # ---------------------------------------------------------------------------
    if FINITE_BETA:
        if PRES_SCALE <= 0.0:
            raise ValueError(
                f"--finite-beta requires --pres-scale > 0 (the default input "
                f"is a vacuum equilibrium); got pres_scale={PRES_SCALE:g}.")
        vmec.indata.pmass_type = 'power_series'
        vmec.indata.gamma = 0.0
        _am = np.asarray(vmec.indata.am, dtype=np.float64).copy()
        _am[:] = 0.0
        _am[:len(PRESSURE_AM_SHAPE)] = PRESSURE_AM_SHAPE
        vmec.indata.am[:] = _am
        vmec.indata.pres_scale = PRES_SCALE
        proc0_print(
            f"  Finite-beta ON: pmass=power_series, "
            f"am[:{len(PRESSURE_AM_SHAPE)}]={PRESSURE_AM_SHAPE}, "
            f"pres_scale={PRES_SCALE:g} Pa  ->  p(s) = pres_scale*(1 - s)")

    # ---------------------------------------------------------------------------
    # QA equilibrium objective + RAW inequality constraints
    # ---------------------------------------------------------------------------
    ...
    # multiplier can decay). Each vmec quantity calls v.run() internally;
    # MaxElongationPen reads the wout, so v.run() is forced first.
    def _aspect_g(v):
        return float(v.aspect()) - ASPECT_TARGET

    def _iota_g(v):
        # iota >= IOTA_TARGET  <=>  IOTA_TARGET - iota <= 0
        return IOTA_TARGET - float(v.mean_iota())
    ...
    def _well_g(v):
        # W >= VACUUM_WELL_TARGET  <=>  VACUUM_WELL_TARGET - W <= 0
        return VACUUM_WELL_TARGET - float(v.vacuum_well())

    def _mercier_g(v):
        # Mercier interchange stability: VMEC's DMerc > 0 is STABLE. The
        # binding constraint is the least-stable INTERIOR surface (drop the
        # axis and edge entries, [1:-1], where DMerc is unreliable — matching
        # diagnose_equilibrium.py). Require min interior DMerc >= margin:
        #   g = MERCIER_MARGIN - min(DMerc[1:-1]) <= 0.
        # NOTE: DMerc has a large dynamic range; PHR handles the scale per
        # constraint (its own rho_i / lambda_i), but tune --mercier-margin if
        # the multiplier trajectory looks ill-conditioned.
        v.run()
        dmerc = np.asarray(v.wout.DMerc, dtype=np.float64)[1:-1]
        return MERCIER_MARGIN - float(np.min(dmerc))

    aspect_g_opt = make_optimizable(_aspect_g, vmec)
    iota_g_opt   = make_optimizable(_iota_g, vmec)
    elong_g_opt  = make_optimizable(_elong_g, vmec)
    well_g_opt   = make_optimizable(_well_g, vmec)

    OBJECTIVES = [
        Objective(fn=qs.residuals, target=0.0, label="QS"),
    ]

    INEQ_CONSTRAINTS = [
        IneqConstraint(fn=aspect_g_opt.J, label="aspect"),
        IneqConstraint(fn=iota_g_opt.J,   label="iota"),
        IneqConstraint(fn=elong_g_opt.J,  label="elong"),
        IneqConstraint(fn=well_g_opt.J,   label="well"),
    ]
    # Finite-beta: add the Mercier interchange stability criterion as an extra
    # PHR inequality constraint. The PHR state arrays (lam_eq, rho_eq) and all
    # diagnostics/plots are sized from n_eq_con, so this extends transparently.
    if FINITE_BETA:
        mercier_g_opt = make_optimizable(_mercier_g, vmec)
        INEQ_CONSTRAINTS.append(
            IneqConstraint(fn=mercier_g_opt.J, label="mercier"))
    ...
    # No coil-force constraint (matches qss_script_jax_qa.py coil targets).
    al2_c_list = _coil_objs["c_list"]
    ...
    if _qss_args.run_dir:
        ...
        OUT_DIR = os.path.join(
            _script_dir, "output",
            f"qss_jax_qa_phr_{_constraint_str}_{_timestamp}", "")
    ...
    proc0_print(f"  Constraints (g <= 0): aspect<={ASPECT_TARGET}, "
                f"iota>={IOTA_TARGET}, elong<={ELONGATION_THRESHOLD}, "
                f"well>={VACUUM_WELL_TARGET}")
    ...
    if FINITE_BETA:
        proc0_print(f"  Finite-beta:  pres_scale={PRES_SCALE:g} Pa, "
                    f"Mercier margin={MERCIER_MARGIN:g} "
                    f"(constraint: min interior DMerc >= margin)")
    else:
        proc0_print(f"  Finite-beta:  OFF (vacuum; no Mercier constraint)")
    proc0_print("")
    ...
    # ---------------------------------------------------------------------------
    qs_residuals_init = qs.residuals()
    qs_total_init     = float(np.sum(qs_residuals_init ** 2))
    ...
    beta_init         = float(vmec.wout.betatotal)
    if FINITE_BETA:
        _dmerc_init = np.asarray(vmec.wout.DMerc, dtype=np.float64)[1:-1]
        dmerc_min_init = float(np.min(_dmerc_init))
        dmerc_unstable_frac_init = float(np.mean(_dmerc_init < 0.0))
    ...
    proc0_print(f"  {'QS residual':<20s}{qs_total_init:>20.6e}")
    ...
    proc0_print(f"  {'Plasma beta':<20s}{beta_init:>20.6e}")
    if FINITE_BETA:
        proc0_print(f"  {'Min interior DMerc':<20s}{dmerc_min_init:>20.6e}"
                    f"  ({'UNSTABLE' if dmerc_min_init < 0 else 'stable'})")
        proc0_print(f"  {'DMerc unstable frac':<20s}"
                    f"{dmerc_unstable_frac_init:>20.2%}")
    ...
    if mpi.proc0_world:
        _initial_metrics = {
            ...
            "qs_residual_sum_sq":   qs_total_init,
            "g_eq":                 {label: float(g_eq_init[i])
                                     for i, label in enumerate(EQ_LABELS)},
            "vacuum_well":          vacuum_well_init,
            "beta_total":           beta_init,
            "finite_beta":          FINITE_BETA,
            "pres_scale":           PRES_SCALE if FINITE_BETA else 0.0,
            "area":                 area_init,
            "volume":               volume_init,
            "nfp":                  int(surf.nfp),
        ...
        if FINITE_BETA:
            _initial_metrics["mercier_min_interior"] = dmerc_min_init
            _initial_metrics["mercier_unstable_frac"] = dmerc_unstable_frac_init
    ...
    # the MPI 2 GiB message limit for the residual vector. Kept identical to
    # qss_script_jax_qa.py for the A/B comparison.
    diff_method                = "forward"
    ...
    for step in range(0 if SKIP_SURFACE_OPT else N_CONTINUATION_STEPS):
        ...
        # Grow VMEC resolution with the continuation step (as in qss_script_jax.py)
        ...
        for al_iter in range(AL_OUTER_ITER):
            ...
            # 1. Build the PHR AL1 subproblem (QA objective + shifted-hinge
    ...
    _final_solver = auglag_solver(
        surface=surf, maxiter=al2_maxiter,
        max_iter_auglag=al2_max_iter_auglag,
        tau=al2_tau, f=f_al2, constraints=_final_objs["c_list"],
        grad_tol=al2_grad_tol, c_tol=al2_c_tol,
        constraint_names=AL2_CONSTRAINT_NAMES,
    )
    _final_solver.solve()
    ...
    # Final QS + raw constraint metrics
    qs_residuals_final = qs.residuals()
    qs_total_final = float(np.sum(qs_residuals_final**2))
    g_eq_final = np.array([con.fn() for con in INEQ_CONSTRAINTS],
                          dtype=np.float64)
    beta_final = float(vmec.wout.betatotal)
    if FINITE_BETA:
        _dmerc_final = np.asarray(vmec.wout.DMerc, dtype=np.float64)[1:-1]
        dmerc_min_final = float(np.min(_dmerc_final))
        dmerc_unstable_frac_final = float(np.mean(_dmerc_final < 0.0))
    ...
    proc0_print(f"  QS residual:     {qs_total_final:.6e}")
    for i, label in enumerate(EQ_LABELS):
    ...
    proc0_print(f"  Plasma beta:     {beta_final:.6e}")
    if FINITE_BETA:
        proc0_print(f"  Min interior DMerc: {dmerc_min_final:.6e}"
                    f"  ({'UNSTABLE' if dmerc_min_final < 0 else 'stable'})")
        proc0_print(f"  DMerc unstable frac: {dmerc_unstable_frac_final:.2%}")
    ...
    if mpi.proc0_world:
        final_metrics = {
            "surface": {
                ...
                "qs_residual_sum_sq": qs_total_final,
                "g_eq": {label: float(g_eq_final[i])
                         for i, label in enumerate(EQ_LABELS)},
                "vacuum_well": float(vmec.vacuum_well()),
                "beta_total": beta_final,
                "mercier_min_interior": (dmerc_min_final if FINITE_BETA
                                         else None),
                "mercier_unstable_frac": (dmerc_unstable_frac_final
                                          if FINITE_BETA else None),
            ...
            "optimization": {
                ...
                "vacuum_well_target": VACUUM_WELL_TARGET,
                "finite_beta": FINITE_BETA,
                "pres_scale": PRES_SCALE if FINITE_BETA else 0.0,
                "mercier_margin": MERCIER_MARGIN if FINITE_BETA else None,
            },
            "qs_parameters": {
                "helicity_m": 1,
                "helicity_n": 0,
                "surfaces": np.arange(0, 1.01, 0.1).tolist(),
            },
        }
        ...
        if len(J1_J2_history) > 0:
            ...
            try:
                ...
                ax.plot(iters, J1_vals, marker="o", label="J1 (QA equilibrium)",
                        color="tab:blue")
                ...
                ax.set_yscale("log")
                ax.set_title("J1 (QA) / J2 (coil) vs surface iteration [PHR]")
                ax.grid(True, which="both", alpha=0.3)
                ax.legend()
